...
//! same network can see each other without scanning a QR (PC → PC connection).
//!
...
/// `own_ip` is checked each packet so the host doesn't list itself. The thread
/// runs until `stop` is set (or the process exits).
